...
from apps.apartments_analyzer.models import SUBWAY_DISTANCES_FIELD, RentApartment, SoldApartments
...
SUBWAY_POINTS = [
    (subway_station, coords)
    for subway_station, coords in SUBWAY_DATA.items()
]
METERS_IN_KM = 1000
    ...
    def update_distance_data_for_apartments(self):
        ...
        with tqdm(total=total_apartments, disable=not self.show_progress) as counter:
            for apartment in apartments:
                apartment = self.add_subway_distance_to_apartment(
                    apartment
                )
                apartment.save()
                counter.update(1)
    ...
    def add_subway_distance_to_apartment(self, apartment):
        ...
        apartment.subway_distances = {
            SUBWAY_DISTANCES_FIELD: [{
                'subway': subway,
                'distance': distance * METERS_IN_KM
            } for subway, distance in distances]
        }
        return apartment

    def distance_between_points(self, point_1: Tuple[float, float], point_2: Tuple[float, float]) -> float:
        return vincenty(point_1, point_2)

    def get_subway_distances_for_coordinate(self, latitude: float, longitude: float) -> List[SubwayDistance]:
        """
        ...
        distances = [
            (station, self.distance_between_points(point_of_interest, subway_coordinates))
