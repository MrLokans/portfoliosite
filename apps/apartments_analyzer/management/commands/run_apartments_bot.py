...
class TelegramSearchRepository:
    ...
    def update_search_price_range(self, contact_id: str, min_price: int, max_price: int):
...
class ApartmentReporterBot:
    ...
    def change_max_price(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            return MAX_PRICE
        self.repo.update_search_price_range(update.message.from_user.id, min_price, max_price)
        update.message.reply_text(
    ...
    def change_room_count(self, update: Update, context: CallbackContext):
        try:
            ...
            return ROOM_COUNT
        self.repo.update_min_room_count(update.message.from_user.id, min_room_count=room_count)
        update.message.reply_text(
    ...
    def _init(self):
        ...
        conversation_handler = ConversationHandler(
            entry_points=[
                CommandHandler(self._START_COMMAND, self.greet_user)
            ],
            states={
                CHOOSING_OPTIONS: [
                    MessageHandler(Filters.regex(f'^{MenuTitle.CHANGE_PRICE}$'), self.change_price_settings),
                    MessageHandler(Filters.regex(f'^{MenuTitle.SHOW_SETTINGS}$'), self.show_settings),
                    MessageHandler(Filters.regex(f'^{MenuTitle.ROOM_COUNT}$'), self.change_room_settings),
                    MessageHandler(Filters.regex(f'^{MenuTitle.DISABLE_SEARCH}$'), self.disable_search),
                    MessageHandler(Filters.regex(f'^{MenuTitle.ENABLE_SEARCH}$'), self.enable_search),
                ],
