...
class ApartmentDataImporter:
    ...
    def _get_existing_apartment_urls(self) -> Set[str]:
        ...
        existing_apartments = itertools.chain(
            RentApartment.objects.urls(),
            SoldApartments.objects.urls(),
        )
    ...
    def _handle_database_sync(self,
        ...
        self._run_stats['total_inactive'] = (
                RentApartment.objects.mark_inactive(inactive_rented_urls) +
        ...
        for item in apartments_to_update:
            ap = self.model_for_url(item['origin_url']).objects.get(bullettin_url=item['origin_url'])
            self._attempt_saving_item(item,
                                      update=True, instance=ap)
        active_rented_urls, active_sold_urls = self.get_urls_by_type(self.active_urls)
        self._run_stats['total_active'] = (
                RentApartment.objects.mark_active(active_rented_urls) +
